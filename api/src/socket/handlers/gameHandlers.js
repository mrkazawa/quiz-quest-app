...
function handleStartQuiz(socket, io, data) {
  ...
  RoomService.setQuestionTimer(roomId, (roomId, ioInstance) => endQuestion(roomId, ioInstance), currentQuestionObj.timeLimit, io);
}
...
function endQuestion(roomId, io) {
  const room = RoomService.getRoom(roomId);
  if (!room) return;
...
function moveToNextQuestion(roomId, io) {
  ...
  console.log(`➡️ Moving to question ${result.currentQuestionIndex + 1}/${result.totalQuestions} in room ${roomId}`);
  ...
  RoomService.setQuestionTimer(roomId, (roomId, ioInstance) => endQuestion(roomId, ioInstance), nextQuestionObj.timeLimit, io);
}

function endQuiz(roomId, io) {
...
function handleGetQuizRankings(socket, io, data) {
  const { roomId } = data;
