...
export default function StudentQuizRoom() {
  ...
  useEffect(() => {
    ...
    const handleQuestionEnded = (data: QuestionResults) => {
      ...
      const playerAnswer = data.playerAnswers?.find(
        (a: PlayerAnswer) => a.playerId === socket?.id
      );
      ...
      if (playerAnswer) {
        ...
        const selectedAnswerText =
          playerAnswer.answerId !== null && data.options
        ...
        if (parsedFinalData?.historyId) {
          try {
            const response = await fetch(
              `/api/quiz-history/${parsedFinalData.historyId}`
            );
            ...
            if (
              historyData.rankings &&
              (currentStudentId || currentPlayerName)
            ) {
              const playerRanking = historyData.rankings.find(
                (ranking: {
                  studentId: string;
                  playerName: string;
                  rank: number;
                }) =>
                  ranking.studentId === currentStudentId ||
                  ranking.playerName === currentPlayerName
              );
                        ...
                        <>
                          {(() => {
                            ...
                            const colorSet = colors[
                              answerResult.selectedAnswer as keyof typeof colors
                            ] || {
                              bg: "bg-gray-500",
                              text: "text-gray-700",
                              light: "bg-gray-100",
                            ...
                            return (
                              <>
                                ...
                                <span className="font-medium text-gray-900">
                                  {answerResult.selectedAnswerText}
                                </span>
                              </>
                            );
                          })()}
              ...
              <div className="mb-8 p-6 bg-yellow-50 rounded-lg">
                ...
                {loadingRanking ? (
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">
                      out of {studentRanking.totalPlayers} player
                      {studentRanking.totalPlayers !== 1 ? "s" : ""}
                    </p>
